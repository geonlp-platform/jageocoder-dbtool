...
Jageocoder 用の住所データベースは [Jageocoder データファイル一覧](https://www.info-proto.com/static/jageocoder/latest/) からダウンロードできますが、これらのデータファイルは国の機関が公開しているオープンデータから作成しているため、市販されている地図データに比べると精度が低かったり、網羅範囲が狭いことがあります。有償の地図データをお持ちであったり、独自に調査して作成した地図データから地図に対応する住所データベースを作りたい場合に、本ソフトを利用してください。

**対象としない利用者**
一般的な住所を解析したり、対応する経緯度を取得したい場合は、本ソフトを利用して住所データベースを作る必要はありません。[Jageocoder データファイル一覧](https://www.info-proto.com/static/jageocoder/latest/) から最新のファイルをダウンロードしてください。
